...
import { shoppingService, BannerPlacement, BannerStatus } from "@mymeddevices/shared-core";
...
export default function NewBannerPage() {
  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      try {
        const data = await shoppingService.getCoupons(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await shoppingService.createBanner({
        ...formData,
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-5xl mx-auto px-4 md:px-6 py-4">
        <div className="flex items-center gap-4 border-b border-border pb-4">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-foreground">Create Banner</h1>
            <p className="text-xs text-muted-foreground">
              Create a new promotional banner or advertisement.
            </p>
          </div>
        </div>
