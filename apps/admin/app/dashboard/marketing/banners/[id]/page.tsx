...
import { 
  ...
  Palette, CheckCircle2, ChevronRight, Eye, Trash2
} from "lucide-react";
...
export default function EditMarketingBannerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        const data: any = await shoppingService.getBanner(resolvedParams.id);
        if (data) {
          setFormData({
            title: data.title || "",
            ...
            target_url: data.target_url || data.cta_link || "/offers",
            button_text: data.button_text || data.cta_text || "Shop Now",
            placement: data.placement || "header_bar",
            bg_color: data.background_color || "#0066cc",
            text_color: data.text_color || "#ffffff",
            priority: data.priority ? String(data.priority) : "1",
            status: data.status || "active",
            start_time: data.start_time ? new Date(data.start_time).toISOString().split('T')[0] : "",
            end_time: data.end_time ? new Date(data.end_time).toISOString().split('T')[0] : "",
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await shoppingService.updateBanner(resolvedParams.id, payload);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold tracking-tight">Edit Banner: {formData.title}</h1>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Modify promotional banner settings and view real-time customer site preview.
              </p>
            </div>
          </div>
                ...
                {PLACEMENT_OPTIONS.map((item) => {
                  ...
                  return (
                    <button
                      key={item.id}
                      type="button"
                      onClick={() => setFormData({ ...formData, placement: item.id })}
                      className={cn(
            ...
            <div className={cn(
              ...
              <div className="bg-background text-foreground font-sans text-xs">
                ...
                <div className="p-4 bg-muted/20 min-h-[220px] flex flex-col justify-start gap-3">
                  ...
                  {/* Hero Slider Placement Preview */}
                  {formData.placement === "homepage_hero" && (
                    <div
                      style={{ backgroundColor: formData.bg_color, color: formData.text_color }}
                      className="relative overflow-hidden rounded-xl border border-border shadow-md min-h-[160px] flex flex-col justify-end p-5 transition-all"
                    >
                      {formData.image_url && (
                        <div
                      ...
                      <div className="relative z-10 space-y-1.5">
                        <span className="text-[10px] uppercase tracking-wider font-semibold opacity-80 px-2 py-0.5 rounded bg-black/20">
                          Featured Campaign
                        </span>
                        <h3 className="text-base font-bold leading-tight">
                          {formData.title || "Homepage Hero Title Preview"}
                        </h3>
                        {formData.description && (
                        ...
                        {formData.button_text && (
                          <button
                            type="button"
                            className="inline-flex items-center text-xs font-semibold px-3 py-1.5 rounded-md shadow-xs mt-2 bg-white text-slate-900"
                          >
                            {formData.button_text} →
                          </button>
                        )}
