...
async def get_user_stats(
    ...
    active_customers_result = await db.execute(
        select(func.count(User.id)).where(
            and_(User.role == "customer", User.is_active == True)
        )
    )
    ...
    active_vendors_result = await db.execute(
        select(func.count(VendorProfile.id)).where(
    ...
    total_staff_result = await db.execute(
        select(func.count(User.id)).where(
            or_(User.role == "admin", User.role == "worker")
        )
    )
...
async def list_staff(
    ...
    role_filter: Optional[str] = Query(None, description="Filter by role: admin, worker"),
...
async def create_staff(
    ...
    logger.info(f"Staff member {email} created by {current_user.email}. Temporary password: {temp_password}")

    return success_response({
        "id": str(new_user.id),
        "email": new_user.email,
        "role": new_user.role,
        "message": "Staff member created successfully. Invite email queued."
    })
...
async def update_staff_status(
    ...
    return success_response({
        "message": f"Staff member {action}d successfully",
        "status": "active" if user.is_active else "inactive"
    })
...
async def list_vendors_overview(
    ...
    query = select(User).where(User.role == "vendor").options(
    ...
    if status_filter:
        query = query.join(VendorProfile).where(
            VendorProfile.approval_status == status_filter
        )
