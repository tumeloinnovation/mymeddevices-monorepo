...
class DarajaService:
    ...
    # A live STK push expires on Safaricom's side after ~90 seconds; a pending
    # payment record older than this is treated as stale so customers can retry.
    PENDING_PAYMENT_STALE_AFTER = timedelta(minutes=3)

    # Callback authentication domain separator (binds tokens to the M-Pesa callback use case)
    _CALLBACK_TOKEN_CONTEXT = "mpesa-callback"
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.callback_url = settings.MPESA_CALLBACK_URL or "https://api.mymeddevices.com/api/v1/payments/mpesa/callback"
        self.timeout = getattr(settings, "MPESA_TIMEOUT_SECONDS", 15.0)

    @classmethod
    def generate_callback_token(cls, order_id: uuid.UUID) -> str | None:
        """
        Derive the per-order callback token appended to the CallbackURL.

        Safaricom cannot sign callback bodies or set custom headers, so the URL
        itself must carry proof of knowledge of MPESA_CALLBACK_SECRET. Binding
        the HMAC to the order ID means a leaked token for one order cannot be
        replayed against any other order. Returns None when no secret is
        configured (development/sandbox; forbidden in production by config
        validation).
        """
        secret = settings.MPESA_CALLBACK_SECRET
        if not secret:
            return None
        message = f"{cls._CALLBACK_TOKEN_CONTEXT}:{order_id}".encode()
    ...
    async def initiate_stk_push(
        ...
        The amount charged is always the full server-side order total; it is
        never accepted from the client. Only orders still awaiting payment may
        be charged, and at most one live STK push per order is allowed.
        """
        ...
        if order.status != OrderStatus.PENDING:
            raise ValueError(f"Order is not awaiting payment (current status: {order.status.value})")

        # Block a second concurrent STK push: the customer could otherwise be
        # double-charged if both prompts are completed.
        now = datetime.now(UTC)
        pending_stmt = select(MobileMoneyPayment).where(
            MobileMoneyPayment.order_id == order.id,
            MobileMoneyPayment.provider == "mpesa",
            MobileMoneyPayment.status == MobileMoneyPaymentStatus.PENDING,
        )
        pending_payments = (await self.db.execute(pending_stmt)).scalars().all()
        for pending in pending_payments:
            if pending.created_at.tzinfo is None:
                pending_age = now - pending.created_at.replace(tzinfo=UTC)
            else:
                pending_age = now - pending.created_at
            if pending_age < self.PENDING_PAYMENT_STALE_AFTER:
                raise ValueError(
                    "An M-Pesa payment is already pending for this order. Complete or cancel it before starting a new one."
                )

        final_amount = order.total_amount
        # Amount must be integer KES for Safaricom Daraja STK Push
        numeric_amount = int(final_amount.quantize(Decimal("1"), rounding=ROUND_HALF_UP))
        if numeric_amount <= 0:
        ...
        callback_token = self.generate_callback_token(order.id)
