...
router = APIRouter(tags=["M-Pesa Payments"])
...
async def initiate_mpesa_stk_push(
    ...
    # 3. Call Daraja Service (amount is always the server-side order total)
...
async def mpesa_stk_callback(
    ...
    Safaricom cannot sign the body or set headers, so when MPESA_CALLBACK_SECRET
    is configured (mandatory in production) the callback must present the
    per-order token that was embedded in the CallbackURL at initiation — an
    HMAC of the secret bound to the order being paid. This proves knowledge of
    the secret and prevents replaying one order's callback against another.
    """
    try:
    ...
    if settings.MPESA_CALLBACK_SECRET:
        checkout_req_id = (body.get("Body") or {}).get("stkCallback", {}).get("CheckoutRequestID")
        payment_stmt = select(MobileMoneyPayment).where(MobileMoneyPayment.transaction_id == checkout_req_id)
        payment = (await db.execute(payment_stmt)).scalar_one_or_none()
        expected_token = DarajaService.generate_callback_token(payment.order_id) if payment else None
        provided_token = token or x_callback_token
        if (
            not provided_token
            or not expected_token
            or not hmac.compare_digest(provided_token.encode("utf-8"), expected_token.encode("utf-8"))
        ):
            client_ip = request.client.host if request.client else "unknown"
            logger.warning(f"Rejected unauthenticated M-Pesa callback from IP {client_ip}: Invalid callback token")
            return {"ResultCode": 1, "ResultDesc": "Unauthorized: Invalid callback token"}
    ...
    try:
        ...
        if res.get("status") in ("ignored", "not_found"):
            return {"ResultCode": 1, "ResultDesc": res.get("reason", "Callback not processed")}
    except Exception as e:
        ...
        try:
            ...
            pass
        return Response(
            content=json.dumps({"ResultCode": 1, "ResultDesc": "Internal processing error"}),
