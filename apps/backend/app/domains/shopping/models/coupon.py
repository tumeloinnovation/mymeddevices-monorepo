...
class Coupon(Base, IDMixin, AuditMixin):
    ...
    discount_scope: Mapped[str] = mapped_column(
        String(50), default="cart", nullable=False
    ...
    restrictions: Mapped[Optional["CouponRestriction"]] = relationship(
        "CouponRestriction", back_populates="coupon", cascade="all, delete-orphan", uselist=False, lazy="selectin"
    )
    ...
    user_coupons: Mapped[list["UserCoupon"]] = relationship(
        "UserCoupon", back_populates="coupon", cascade="all, delete-orphan"
    )
    categories: Mapped[list["CouponCategory"]] = relationship(
        "CouponCategory", back_populates="coupon", cascade="all, delete-orphan", lazy="selectin"
    )
    products: Mapped[list["CouponProduct"]] = relationship(
        "CouponProduct", back_populates="coupon", cascade="all, delete-orphan", lazy="selectin"
    )
...
class CouponRestriction(Base, IDMixin):
    ...
    coupon_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("coupons.id", ondelete="CASCADE"), unique=True, nullable=False, index=True
    )
    ...
    one_time_per_user: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    global_usage_limit: Mapped[int | None] = mapped_column(Integer, nullable=True)
    vendor_only: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    exclude_sale_items: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)

    # Buy X Get Y details
    buy_product_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("products.id", ondelete="SET NULL"), nullable=True
    )
    buy_quantity: Mapped[int | None] = mapped_column(Integer, nullable=True)
    get_product_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("products.id", ondelete="SET NULL"), nullable=True
    )
    get_quantity: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # Relationships
    coupon: Mapped["Coupon"] = relationship("Coupon", back_populates="restrictions")
    buy_product: Mapped[Optional["Product"]] = relationship("Product", foreign_keys=[buy_product_id], lazy="selectin")
    get_product: Mapped[Optional["Product"]] = relationship("Product", foreign_keys=[get_product_id], lazy="selectin")
...
class CouponUsage(Base, IDMixin):
    ...
    used_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=func.now(), nullable=False)
...
class UserCoupon(Base, IDMixin):
    ...
    assigned_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.now(UTC), nullable=False)
...
class CouponCategory(Base, IDMixin):
    ...
    # Relationships
    coupon: Mapped["Coupon"] = relationship("Coupon", back_populates="categories")
...
class CouponProduct(Base, IDMixin):
    ...
    # Relationships
    coupon: Mapped["Coupon"] = relationship("Coupon", back_populates="products")
