...
class CartMergeLog(Base, IDMixin, AuditMixin):
    ...
    source_cart_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("carts.id", ondelete="SET NULL"),
        nullable=True
    )  # Guest cart being merged

    target_cart_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("carts.id", ondelete="SET NULL"),
        nullable=True
    ...
    source_item_count: Mapped[int] = mapped_column(
        ...
        nullable=False,
        default=0
    )  # Number of items in source cart
