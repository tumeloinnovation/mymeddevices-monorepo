...
async def admin_list_coupons(
    ...
    coupons, total = await service.list_coupons(only_active=only_active, offset=(page - 1) * page_size, limit=page_size)
...
async def admin_create_coupon(
    ...
    service = CouponService(db)
    try:
        coupon = await service.create_coupon(**data.model_dump(), created_by_id=current_user.id)
...
async def admin_list_abandoned_carts(
    ...
    stmt = (
        select(Cart)
        .where(and_(Cart.updated_at <= threshold_time, Cart.is_active == True, Cart.user_id.isnot(None)))
...
async def admin_shopping_analytics(
    ...
    recent_orders_result = await db.execute(select(func.count(Order.id)).where(Order.created_at >= thirty_days_ago))
    ...
    abandoned_carts_result = await db.execute(
        select(func.count(Cart.id)).where(
            and_(Cart.is_active == True, Cart.updated_at <= (now - timedelta(hours=24)), Cart.user_id.isnot(None))
        )
    )
    ...
    recent_revenue_result = await db.execute(
        select(func.coalesce(func.sum(Order.total_amount), 0)).where(Order.created_at >= thirty_days_ago)
    )
    ...
    status_breakdown = {row[0]: int(row[1]) for row in status_breakdown_result}
