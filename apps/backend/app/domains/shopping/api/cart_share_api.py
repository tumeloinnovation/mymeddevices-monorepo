...
async def create_cart_share(
    ...
    share = await service.create_share(cart_id, current_user.id, share_in.expires_days or 7)
...
async def get_shared_cart(token: str, db: AsyncSession = Depends(get_db)):
    ...
    for item in cart.items:
        if not item.product or item.product.price is None:
            continue
        ...
        items.append(
            {
                ...
                "notes": item.notes,
                "product": {"id": item.product.id, "name": item.product.name, "price": price, "sku": item.product.sku},
            }
        )
