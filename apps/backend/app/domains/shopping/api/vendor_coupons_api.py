...
async def get_vendor_profile(user: User, db: AsyncSession) -> Optional[VendorProfile]:
...
async def vendor_get_coupon(
    ...
    if coupon.vendor_id != vendor.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only view your own coupons"
        )
...
async def vendor_update_coupon(
    ...
    if coupon.vendor_id != vendor.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only update your own coupons"
        )
...
async def vendor_delete_coupon(
    ...
    if coupon.vendor_id != vendor.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only delete your own coupons"
        )
...
async def vendor_get_coupon_stats(
    ...
    if coupon.vendor_id != vendor.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only view stats for your own coupons"
        )
    ...
    total_uses_stmt = select(func.count()).select_from(
        select(CouponUsage.id).where(
            and_(
                ...
                CouponUsage.vendor_id == vendor.id
            )
        ).subquery()
    )
