...
async def save_current_cart(
    ...
    source_cart_id: uuid.UUID | None = Query(None),
    db: AsyncSession = Depends(get_db),
):
    """Save the current cart for future use."""
    service = SavedCartService(db)
    saved_cart = await service.create_saved_cart(
        user_id=current_user.id, name=data.name, description=data.description, source_cart_id=source_cart_id
    )
...
async def list_my_saved_carts(
    ...
    saved_carts = await service.list_saved_carts(
        user_id=current_user.id, offset=(page - 1) * page_size, limit=page_size
    )
...
async def restore_saved_cart(
    ...
    cart = await service.restore_to_cart(saved_cart_id=saved_cart_id, target_cart_id=target_cart_id, replace=replace)
    return success_response(
        {"message": "Cart restored successfully", "cart_id": cart.id, "item_count": len(cart.items)}
    )
