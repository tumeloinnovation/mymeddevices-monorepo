...
class WishlistToCartRequest(BaseModel):
    ...
    wishlist_item_ids: list[uuid.UUID] = Field(..., description="List of wishlist item IDs to move to cart")
    action: str = Field(
        default="move", description="Action: 'move' (remove from wishlist) or 'copy' (keep in wishlist)"
    )
...
class CartToWishlistRequest(BaseModel):
    ...
    remove_from_cart: bool = Field(
        default=True, description="Whether to remove item from cart after adding to wishlist"
    )
...
class ComparisonToCartRequest(BaseModel):
    ...
    comparison_item_ids: list[uuid.UUID] = Field(..., description="List of comparison item IDs to add to cart")
    quantities: dict | None = Field(default=None, description="Optional mapping of comparison_item_id to quantity")
